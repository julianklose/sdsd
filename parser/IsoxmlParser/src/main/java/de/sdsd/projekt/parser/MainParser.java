...
public class MainParser {
	...
	public static String getDdiDesignator(int ddi) {
		try {
			if(ddiMap == null) ddiMap = ParserAPI.getWikinormiaInstances(Util.UNKNOWN.res("ddi"), true);
			WikiInstance wikiddi = ddiMap.get(Integer.toString(ddi));
			if(wikiddi != null) return wikiddi.label;
	...
	public static ElementType elementtype(Geo.GeoType type) {
		if(type == Geo.PolygonType.PARTFIELD_BOUNDARY) return ElementType.Field;
		else if(type == Geo.PolygonType.TREATMENT_ZONE) return ElementType.TreatmentZone;
		else if(type == Geo.LineStringType.GUIDANCE_PATTERN) return ElementType.GuidancePattern;
		else if(type == Geo.PointType.FIELD_ACCESS) return ElementType.FieldAccess;
		...
		try {
			...
			for(IsoXmlElement tsk : taskdata.findChildren("TSK")) {
				...
				for(IsoXmlElement child : tsk.getChildren()) {
					try {
						...
						e.printStackTrace();
						errors.add(child.prefixEnd(e.getMessage()));
					}
				}
				...
				for(int i = 0; i < tims.size(); ++i) {
					...
					Instant start = tim.getAttribute("start", DatetimeAttr.class).getValue(), stop;
					{
						...
						if(stopattr.hasValue()) stop = stopattr.getValue();
					...
					for(TimedTimeLog t : tsktlgs) {
						if(t.start.isBefore(start) || t.start.isAfter(stop)) continue;
						tlg = t.tlg;
						break;
					}
					...
					for(IsoXmlElement dlv : tim.findChildren("DLV")) {
						try {
							...
							if(tlg != null) vd.writeTo(model, ISOXMLParser.FORMAT.res("DLV"));
							else vd.writeToNoParent(model, ISOXMLParser.FORMAT.res("DLV"));
						} catch (SAXException e) {
							errors.add(dlv.prefixEnd(e.getMessage()));
						}
					}
				}
			}
			...
			for(Geo geo : isoxml.getAllGeometries()) {
				try {
					...
					errors.add(geo.element.prefixEnd(e.getMessage()));
				}
			}
			...
			if(features.size() > 0) {
				try (GeoWriter geo = api.writeGeo()) {
					for(GeoObject feature : features) {
						...
						if(element == null) element = feature.geo.element; // shouldn't be possible
			...
			for(TimeLog timelog : timelogs) {
				...
				try (TimeLogWriter tlw = api.addTimeLog(timelog.getTimeLog(), valueInfos)) {
					for(TimeLogEntry entry : timelog) {
						...
						double altitude = posUp && entry.hasValue(3) ? entry.getHead(3, Integer.class) / 1000. : Double.NaN;
				...
				try (GridWriter gw = api.addGrid(grid.getGrid(), grid.getRowCount(), grid.getColumnCount())) {
					...
					for(Grid.ValueInfo info : infos.values()) {
						...
						for(int row = 0; row < grid.getRowCount(); ++row) {
							...
							if(!empty) gw.writeGridRow(values);
						}
					}
				}
			}
	...
	public static void validate(InputStream input, OutputStream output) {
		try (PrintWriter out = new PrintWriter(new OutputStreamWriter(output, StandardCharsets.UTF_8), true)) {
			try {
				...
				taskdata.getAllErrors().forEach(out::println);
				
				try {
					for(Geo geo : isoxml.getAllGeometries()) {
						geo.getErrors().forEach(out::println);
					}
				} catch (Exception e) {
				...
				for(IsoXmlElement tlg : isoxml.getAllTimeLogs()) {
					try {
						...
						timeLog.getAllErrors().forEach(out::println);
					} catch (Exception e) {
						out.println(tlg.prefixEnd(e.getMessage()));
					}
				}
				for(IsoXmlElement grd : isoxml.getAllGrids()) {
					try {
						Grid grid = isoxml.getGrid(grd);
						grid.getAllErrors().forEach(out::println);
					} catch (Exception e) {
						out.println(grd.prefixEnd(e.getMessage()));
					}
				}
			} catch (Throwable e) {
				out.println(e.getMessage());
