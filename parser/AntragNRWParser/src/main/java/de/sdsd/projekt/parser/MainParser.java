		...
		try {
			...
			if(fields.size() > 0) {
				...
				for(Entry<String, SchlagInfo> e : fields.entrySet()) {
					try {
						...
						errors.add(e.getKey() + ": " + e1.getMessage());
					}
				}
			}
			...
			if(geometries.size() > 0) {
				...
				for(Entry<String, Geometry> e : geometries.entrySet()) {
					...
					if(info == null) 
						errors.add("Schlag " + e.getKey() + " not found in Flächenverzeichnis.xml");
					else {
						try {
							...
							features.add(new JSONObject()
									.put("type", "Feature")
									.put("id", info.uri)
									.put("label", info.getName())
									.put("geometry", new JSONObject(geo.toString()))
