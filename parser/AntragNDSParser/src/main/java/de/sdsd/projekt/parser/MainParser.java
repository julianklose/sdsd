				...
				for(SchlagInfo schlag : fields) {
					for(TeilschlagInfo teil : schlag.getTeilschlaege()) {
						try {
							...
							if(geo == null) throw new NullPointerException("Geometry missing");
							geo = JTS.toGeographic(geo, antrag.crs);
							JSONObject feature = new JSONObject()
									.put("type", "Feature")
									.put("id", teil.uri)
									.put("label", teil.getName())
									.put("properties", teil.properties());
							...
							errors.add(schlag.getNummer() + teil.getTeil() + ": " + e1.getMessage());
						}
					}
				}
			}
			...
			if(features != null && !features.isEmpty()) {
				try (GeoWriter geo = api.writeGeo()) {
					for(JSONObject feature : features) {
						geo.writeFeature(feature, ElementType.Field, feature.getString("id"), feature.getString("label"));
					}
				}
			}
		} catch (Throwable e) {
