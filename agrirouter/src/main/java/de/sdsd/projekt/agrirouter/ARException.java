...
public class ARException extends Exception implements Iterable<Message> {
	 ...
	 * @return the detail message (which is saved for later retrieval by the getMessage() method).
	 */
	private static String createMessage(List<Message> messages) {
		...
		else return "Empty agrirouter error";
	}

	/**
	 * Get iterable over every agrirouter error message, that is associated with this exception.
	 ...
	 * @return agrirouter error message or null, if there is no message for the given index
	 */
	...
	/**
	 * Get a list of every agrirouter error message, that is associated with this exception.
	 * @return agrirouter error messages
	 */
	...
	/**
	 * Returns all error messages with error codes from the agrirouter.
	 * Use this for intern logging and {@link #getMessage()} for extern representation of the error.
	 */
	@Override
