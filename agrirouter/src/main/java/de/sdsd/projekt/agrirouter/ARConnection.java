...
/**
 * Agrirouter connection. 
 * This class represents an endpoint specific connection to the agrirouter.
 * You need this for every agrirouter function except of onboarding. 
 *
...
public abstract class ARConnection implements AutoCloseable {
	...
	/**
	 * Construct and open a new connection to the agrirouter.
	 * Make sure to {@link #close()} the connection when you don't need it anymore.
	 * 
	 * @param onboardingResponse json response from the {@link AROnboarding.SecureOnboardingContext#onboard(String)}
	 ...
	 * @throws JSONException when there is an error in the json onboarding response.
	 * @throws GeneralSecurityException error during creation of the keys for client ssl communication
	...
	/**
	 * Set this to receive push notifications from the agrirouter.
	 * Make sure push notifications were enabled in {@link ARCapabilities}.
	 * 
	 ...
	 */
	public <T> CompletableFuture<T> sendRequestAsync(ARRequest<T> request, long timeout, TimeUnit unit) throws IOException, ARException {
	 ...
	 */
	public <T> T sendRequest(ARRequest<T> request, long timeout, TimeUnit unit) throws IOException, InterruptedException, ARException {
		try {
	...
	public static final class MqttConnection extends ARConnection {
		...
		/**
		 * Construct and open a new MQTT connection to the agrirouter.
		 * Make sure to {@link #close()} the connection when you don't need it anymore.
		 * 
		 * @param onboardingResponse json response from the {@link AROnboarding.SecureOnboardingContext#onboard(String)}
		 * @param executor executor service with thread pool to use for receiving responses from the agrirouter. Doesn't need more than one thread.
		 ...
		 * @throws MqttException if the client is not connected or any other problem with the connection
	...
	public static final class RestConnection extends ARConnection {
		...
		/**
		 * Construct and open a new rest connection to the agrirouter.
		 * Make sure to {@link #close()} the connection when you don't need it anymore.
		 * 
		 * @param onboardingResponse json response from the {@link AROnboarding.SecureOnboardingContext#onboard(String)}
		 * @param executor executor service with thread pool to use for receiving responses from the agrirouter. Doesn't need more than one thread.
		 ...
		 */
		public CloseableHttpResponse send(HttpUriRequest request) throws ClientProtocolException, IOException, ARException {
			try {
				resetTimeout();
